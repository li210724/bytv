...
# Repo (this script): https://github.com/li210724/bytv
#
...
#  - Domain A/AAAA points to this server IP
#  - Ports 80/443 open (cloud security group/firewall)
# ==========================================================
...
PORT="${PORT:-3000}"           # host port -> container 3000 (used by nginx proxy)
...
install_nginx_certbot() {
  ...
  case "$pm" in
    apt)
      ...
      ;;
    dnf|yum)
      install_pkg certbot python3-certbot-nginx || install_pkg certbot python3-certbot || true
      ;;
  esac
...
uninstall_all() {
  ...
  rm -f /usr/local/bin/decotv 2>/dev/null || true

  log "卸载完成"
}

install_as_command() {
  local target="/usr/local/bin/decotv"
  local self; self="$(readlink -f "$0" || true)"
  if [[ -z "$self" || ! -f "$self" ]]; then
    err "无法确定脚本路径，建议从文件运行后再安装为命令"
    exit 1
  fi
  cp -f "$self" "$target"
  chmod +x "$target"
