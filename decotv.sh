...
ensure_basic_deps() {
  ...
  # ca-certificates：用证书 bundle 文件判断是否缺失
  [[ -f /etc/ssl/certs/ca-certificates.crt || -f /etc/pki/tls/certs/ca-bundle.crt ]] || pkgs+=("ca-certificates")

  local pm; pm="$(detect_pm)"
  if [[ "$pm" == "apt" ]]; then
    has_cmd gpg || pkgs+=("gnupg")
    has_cmd lsb_release || pkgs+=("lsb-release")
  fi
